...
normaliseWord word = foldl (\acc c -> if isPunctuation c then acc else acc ++ [toLower c]) [] word
...
isMostCommon prev word count = if (snd prev) < count then (word, count) else prev

buildMap :: Set.Set String -> Header -> [String] -> Map.StringMap Int
buildMap skipSet header allText = foldl
    (\acc word -> if isValidWord skipSet "Scotland" word then updateValue acc word else acc)
    Map.empty
    allText
...
isValidWord skipSet header word = (not (all isDigit word)) && (not (prefix `isPrefixOf` word)) && (Set.notMember word skipSet)
   where prefix = map toLower $ take 4 header